...
`Dockerfile.claude` を編集して `./claude-container -b /path/to/project` でリビルドする。`CLAUDE_CODE_VERSION` ビルド引数はデフォルト `latest` のため、リビルドするとその時点の最新版の Claude Code がインストールされる。再現性が必要な場合は `compose.yml` でバージョンを固定する。
...
Edit `Dockerfile.claude` and rebuild with `./claude-container -b /path/to/project`. The `CLAUDE_CODE_VERSION` build arg defaults to `latest`, so a rebuild always installs the latest Claude Code at that point in time. Pin it in `compose.yml` for reproducibility.
